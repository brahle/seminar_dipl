...
const int H = 10; // penalty for the start of the gap
const int G = 2; // penalty for each element of the gap

inline __device__ __host__ int f(char A, char B) {
  return (A==B);
}

__global__ void calculate_T1_and_T3(int *T1_new,
                                    int *T3_new,
                                    int *T1_old,
                                    int *T2_old,
                                    int *T3_old,
  ...
  max_value = BlockReduce::Reduce(smem_storage, max_value, MaxOperator<int>());
  if (threadIdx.x == 0 && *total_max < max_value) {
    *total_max = max_value;
  }
}

__global__ void calculate_T2(int *T2_new,
                             int *T1_new,
                             int *T2_old,
                             int *T3_new,
                             char *B,
  ...
  register int end = min2(ITEMS_PER_THREAD*(threadIdx.x+1), N);
  IndexedValue max_value = {0, 0}, empty = {0, 0}, iv;
  ...
  typedef cub::BlockScan<IndexedValue, THREADS> BlockScan;
  __shared__ typename BlockScan::SmemStorage smem_storage;
  BlockScan::ExclusiveScan(smem_storage, max_value, max_value, empty, MaxOperator<IndexedValue>());

  j = start;
  T2_new[j] = max2(0, max_value.value - j*G);
  ++j;
  for (; j < end; ++j) {
    if (B[j-1] == RESET) {
      T2_new[j] = 0;
...
void read_protein(char *A, int n, const char *S) {
  int m = strlen(S);
  for (int i = 0; i+1 < n; ++i) {
    A[i] = S[rand()%m];
  }
...
int main(void) {
  init();
  read_protein(A, M, "ABCD");
  read_protein(B, N, "ABCDABCDABCDABCDABCD#");
...
//  puts(A); puts(B);
  ...
  copy_to_device(&dev_indexes, indexes, N);
